...
function normalizePos(pos)
    return pos > 100 and 100 or (pos < 0 and 0 or pos)
end
...
function RSS:new(config)
    ...
    local stopReporting = function()
        o.reportTimer:unregister()
    end
    ...
    local openCloseState = function(motor, dir, time)
        return function(state)
            ...
            state.enter = function(target) --trigger leading to this state must have a target position parameter
                targetPos = target
                callback(o.pos, motor, state.name)
            ...
            state.exit = function(newState, exitTrigger)
                ...
                o.pos = (exitTrigger == machine.TRIG_TIMEOUT) and targetPos or currentPos(dir, time, timestamp)
                stopReporting()
            end
            state.triggers = {
    ...
    o.machine =
        machine:new(
        {
            ...
            states = {
                ...
                [STATE_UNDEF_CHECKDIR] = {
                    enter = function(target)
                        ...
                        return target == 100 and STATE_FULLY_CLOSING or
                            (target == 0 and STATE_FULLY_OPENING or STATE_HOMING), target
                    end
                },
                ...
                [STATE_FULLY_OPENING] = fullyOpenCloseState(RSS.MOTOR_STATUS_UP, 0, config.timeUp),
                [STATE_FULLY_CLOSING] = fullyOpenCloseState(RSS.MOTOR_STATUS_DOWN, 100, config.timeDown),
                [STATE_POSITIONED] = {
                    enter = function()
                ...
                [STATE_CHECKDIR] = {
                    enter = function(target)
                        ...
                        return target == o.pos and STATE_POSITIONED or
                            (target > o.pos and STATE_CLOSING or STATE_OPENING), target
                    end
                },
                [STATE_OPENING] = openCloseState(RSS.MOTOR_STATUS_UP, -1, config.timeUp),
                [STATE_CLOSING] = openCloseState(RSS.MOTOR_STATUS_DOWN, 1, config.timeDown)
            }
        }
    )
...
function RSS:setpos(target)
    self.machine:trigger(TRIG_SET_POS, target)
end

function RSS:stop()
    self.machine:trigger(TRIG_STOP)
end
