...
return function(conf)
    ...
    conf.auth.loginCallback = conf.auth.loginCallback or
                                  function(user, password) return false end
    ...
    s:listen(conf.port, function(connection)
        ...
        local function startServing(fileServeFunction, connection, req, args)
            connectionThread = coroutine.create(
                                   function(fileServeFunction,
                                            bufferedConnection, req, args)
        ...
        local function onReceive(connection, payload)
            ...
            if payload:find("Content%-Length:") or bBodyMissing then
                ...
                if (tonumber(string.match(fullPayload, "%d+", fullPayload:find(
                                              "Content%-Length:") + 16)) >
            ...
            if user and req.methodIsValid and
                (req.method == "GET" or req.method == "POST" or req.method ==
                    "PUT") then
                req.user = user
                ...
                if not user then
                    ...
                    args = {headers = {auth.authErrorHeader(conf.auth.realm)}}
        ...
        local function onSent(connection, payload)
            ...
            if connectionThread then
                local connectionThreadStatus =
                    coroutine.status(connectionThread)
